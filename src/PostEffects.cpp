...
//    setBlendParams(BlendParams(BlendFactor::One,
//                               BlendFactor::One,
//                               BlendFactor::One,
...
//    ));
    ...
    // TexMip &first_mip = m_mips.at(0);
    // first_mip.pixels.clear({0, 0, 0, 0});
    // m_downsample_pass.setUniform("u_src_resolution",
    //                              glm::vec2(first_mip.getWidth(), first_mip.getHeight()));
    // m_screen_sprite.draw(first_mip.pixels, m_downsample_pass, source);
    // Texture *prev_texture = &m_mips.at(0).pixels.getTexture();
...
BloomFinal::BloomFinal(int width, int height, int mip_count, int gauss_pass_count,
                       float brightness_threshold, TextureOptions options)
        : m_brightness_pass(std::string(vertex_sprite_code_direct),
                            std::string(fragment_brightness_code)),
          m_gauss_vert_pass(std::string(vertex_sprite_code_direct),
                            std::string(fragment_gauss_vert_code)),
...
void BloomFinal::process(Texture &source, Renderer &target) {
    ...
    for (int pass = 0; pass < m_gauss_pass_count; ++pass) {
        ...
        m_screen_sprite.draw(first_mip.pixels_tmp, m_gauss_vert_pass,
                             first_mip.pixels.getTexture());

        first_mip.pixels.clear({0, 0, 0, 0});
        m_screen_sprite.draw(first_mip.pixels, m_gauss_horiz_pass,
                             first_mip.pixels_tmp.getTexture());
    }
